...
/**
 * The InvokePutCommandTest class.
 * 
 *<p>This test class test put request.
 *
...
public class PutCommandTest {
    /**
     * InvokePutCommandTest instantiate.  
     */
    ...
    private final static Logger logger =
            LoggerFactory.getLogger(PutCommandTest.class);
    /**
    ...
    @Test
    public void
           testWithHystrixPropertiesInitializationForPutMethod() {
        ...
        try {
            String result = new PutCommand(this.restTemplate,
                    "http://localhost:8090/list", this.fallbackFunction,
                    new RequestConfiguration(5000, 7, 0), "dataToUpdate",
                    this.appConfig, headersMap).execute();
            Assert.assertEquals(result, "Gaurav");
        } finally {
            context.shutdown();
        }
    }
    ...
    public void testFallbackScenerio() {
        ...
        try {
            String result = new PutCommand(this.restTemplate,
                    "http://localhost:8090/list", this.fallbackFunction,
                    new RequestConfiguration(5000, 7, 11), "dataToUpdate",
                    this.appConfig, headersMap).execute();
